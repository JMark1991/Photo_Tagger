...
def extract_features(filepath, filenames=[], model='ResNet50', write_to=None):
    ...
    for (i, fp) in enumerate(img_fps):
        print('\rProcessing: {:.3f}%\t\t'.format((i+1) / len(img_fps) * 100), end='', flush=True)
        features.append(_extract(fp, m))
        if i % 4001 == 4000:
            features_df = save_features(features, img_fns, write_to, existing_feat)
...
def save_features(features, img_fns, write_to, existing_feat):
    # Make into a DataFrame and add a#n ID column
    ...
    #print('new_rows ', features_df.shape, features_df.columns)
    #print('old ', existing_feat.shape, existing_feat.columns)
    if existing_feat is not 'Empty':
        features_df = pd.concat([existing_feat, features_df], axis=0)
    
    print('Features.csv new shape: ', features_df.shape)
